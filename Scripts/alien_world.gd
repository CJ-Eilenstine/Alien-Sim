...
var cowList = [
	preload("res://Animation/alien_cow.tscn"),
	preload("res://Animation/strawberry_cow.tscn"),
]
...
func _process(delta: float):
	...
	if _spawns_quantity >= 1 and cowCount < _spawn_limit:
		...
		var spawns_count: int = floor(_spawns_quantity)
		...
		for index in range(spawns_count):
			spawn()
